...
class Baselines(Algorithm):
    """ Makes M1 Baseline Topologies: ER/Scale Free etc."""
...
class CosineReassignment(Algorithm):
    ...
    def compare_communities(self,max_iters=2):
        """ Devices store parameters from both their own model and the community model.  Then compare their model to the community models of their 
        neighbors to decide if the neighbor's is a better fit. Also compare to their own current community model. If no community is better then no switch occur.
        """
        changes=1000
        ...
        for iter in range(max_iters):
            ...
            for dobj in self.device_list:
                ...
                # self.device_list[dvc].color = self.device_list[max_nbr].color # NOTE: color param used in testing/viz, not actually relevant to final
                
                if (max_nbr != dvc):
                    changes += 1
        
        self.assign_communities()  # Re-Set Communities after Comparisons
        return
...
class DPP(Algorithm):
    ...
    def reset_cost_collection(self):
        return super().reset_cost_collection()

    def run_global_round_setup_steps(self, round=0, threshold=10):
        super().update_coordinates(round)
        self.run_dpp_linking(round_num=round, threshold=self.threshold)

    def run_dpp_linking(self, round_num=0, threshold=0.5):
        self.cosim_matrix = [[None for _ in range(100)] for _ in range(100)]
    ...
    def run_spatial_weighted_attachment(self, threshold=0.75, num_rounds=10):
        ...
        self.fast_build_proximity_graph_(threshold) ## NOTE: TODO -- get rid of the NXG1 structure thing altogether
